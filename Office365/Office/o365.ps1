...
function Get-AvailableOfflineLanguages {
    ...
    # Office365/{Mimari} altındaki herhangi bir klasörü bul (genelde ilk dil klasörü)
    # Find any folder under Office365/{Architecture} (usually the first language folder)
    $firstLangDir = Get-ChildItem -Path $architecturePath -Directory -ErrorAction SilentlyContinue | Select-Object -First 1
    ...
    $officeDataPath = Join-Path $firstLangDir.FullName "Office\Data"
...
$offlineInstallButton.Add_Click({
    ...
    $installPath = Get-DownloadPath -version $selectedVersion -architecture $architecture -channel $channel -languages $languageIds -licenseType $licenseType
